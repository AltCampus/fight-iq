...
function updateScore(req, res) {
	User.find({}, "predictions")
		.populate({
			path: "predictions",
			select: "fightid winner type round",
			populate: [
				{
					path: "fightid",
					select: "result",
		...
		.exec((err, users) => {
			...
			users.forEach((singleUser) => {
				var winCorrectCount = 0,
					points = 0,
					accuracy = 0,
					completedEventCount = 0;
				singleUser.predictions.forEach((data) => {
					if (data.fightid.result) {
						// If result present
						completedEventCount++;
				...
				accuracy =
					Math.floor((winCorrectCount / completedEventCount) * 100) || 0;

				User.findByIdAndUpdate(
					singleUser._id,
					{ $set: { points: points, accuracy: accuracy } },
					{ new: true },
					(err, newData) => {
