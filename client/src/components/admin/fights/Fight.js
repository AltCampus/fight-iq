			...
			<div className="Fight">
					...
					Results: 
						{
								fight.result? (
									<div className="fight-result">
										<div className="fight-result-winner">Winner: {fight.result.winner.name}</div>
										<div className="fight-result-type">Type: {fight.result.type}</div>
										<div className="fight-result-round">Round: {fight.result.round}</div>
									</div>
					...
					{/* TODO: make the result buttons display conditional according to results */}
						...
						<Link to={'/admin/' + eventid + '/fights/' + fight._id + '/result'}>
							<i className="far fa-plus-square"></i> Add result
						</Link>
						<a onClick={this.handleResultEdit}>
							<i className="fas fa-edit"></i> Edit result
						</a>
						<a onClick={this.handleResultDelete}>
							<i className="fas fa-trash-alt"></i> Delete result
						</a>
					</div>
			</div>
			);
